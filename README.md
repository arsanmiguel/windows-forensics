...
A comprehensive PowerShell-based diagnostic tool for Windows servers that automatically detects performance bottlenecks and can create AWS Support cases with detailed forensic data. Originally created for AWS DMS migration troubleshooting; run on your SOURCE DATABASE SERVER. If you are still using this for that purpose, be absolutely certain you run it on your source database server (the host where the database under migration actually runs)—not on a jump box, bastion, or the DMS replication instance alone. Now useful for any Windows performance troubleshooting; run on the machine you want to diagnose and optionally open an AWS Support case with full details attached.
...
**Internal review:** CSE scan commands and documented exceptions — [SECURITY-EXCEPTIONS.md](SECURITY-EXCEPTIONS.md). Run `./security-scan.sh` before handoff.
...
If you are migrating with DMS, run these diagnostics on the **same Windows host where the source database process lives** (your SOURCE DATABASE SERVER). Running elsewhere yields misleading or empty results.
...
Use a named profile if you prefer: `aws configure --profile forensics` and set `$env:AWS_PROFILE = "forensics"` before running the script.
...
.\Invoke-WindowsForensics.ps1 -Mode Standard -CreateSupportCase
