...
Function Initialize-OutputFile {
    ...
    $fileName = "windows-forensics-$randomSuffix.txt"
    $script:OutputFile = Join-Path $Directory $fileName
    ...
    try {
        ...
        Set-Acl -LiteralPath $script:OutputFile -AclObject $acl
    } catch {
        Write-Verbose "Unable to restrict output file ACL: $_"
    }
}
...
"@
            ...
            $queryResults = & sqlcmd -S localhost -E -Q $sqlQuery -h -1 -W 2>&1
        ...
        try {
            ...
            $recoveryResults = & sqlcmd -S localhost -E -Q $recoveryCheck -h -1 -W 2>&1
            if ($LASTEXITCODE -eq 0) {
            ...
            $alwaysOnResults = & sqlcmd -S localhost -E -Q $alwaysOnCheck -h -1 -W 2>&1
...
"@
            ...
            $queryResults = & mysql -u root -e $mysqlQuery 2>&1
...
"@
            ...
            $queryResults = & psql -U postgres -c $pgQuery 2>&1
...
"@
            ...
            $queryResults = & mongo --quiet --eval $mongoQuery 2>&1
...
Function Invoke-Forensics {
    ...
    Initialize-OutputFile -Directory $OutputPath
